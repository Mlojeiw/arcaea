...
func show_main_menu_background():
	...
	tween.set_loops()
	tween.tween_property(background,"position",Vector2(0,-10),35)
	tween.tween_property(background,"position",Vector2(0,-700),35)
	tween.tween_interval(5)
func show_start():
	start.anchor_left = 0
	start.anchor_top = 0
	start.position = Vector2(430 ,430 ) 
	start.modulate = Color(1,1,1,0.8)
	move_effect(start,start.position - offset_vec,0.5)
func show_story():
	story.anchor_left = 0
	story.anchor_top = 0
	story.modulate = Color(1,1,1,0.8)
	story.position = Vector2(350 ,580 ) 
	move_effect(story,story.position - offset_vec,0.5)
...
func show_world():
	...
	move_effect(world,Vector2(820 ,450),0.5)
...
func show_course():
	course.position = Vector2(270 ,730 ) 
	course.modulate = Color(1,1,1,0.8)
	move_effect(course,course.position - offset_vec,0.5)
